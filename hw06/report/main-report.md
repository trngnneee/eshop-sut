...
Báo cáo đối soát [`execution-coverage.md`](../newman/reports/execution-coverage.md) trích TC ID trực tiếp từ tên assertion trong mọi Newman JSON: **126/128 = 98.4%** case đã thực thi; 0 Manual và 2 Blocked đều có lý do trong bảng test case.
...
**Test case không thực thi được.** Execution coverage đạt 127/128 (99.2%) — API-1 và API-2 phủ 100%. Bốn case từng bị đánh Blocked/Manual nay đã chạy thật: `TC-API-LOGIN-042` và `TC-API-CHECKOUT-029` tự ký JWT bằng secret vốn đã công khai tại `backend/server.js:9` (suite `04-jwt-cases`), `TC-API-LOGIN-041` và `TC-API-LOGIN-024` chạy sau khi chờ lock 180 giây hết hạn (suite `05-timed-assert`). Riêng `TC-API-ORDER-STATUS-041` vẫn `Blocked` vĩnh viễn vì SUT **không có** endpoint dashboard/revenue nào để quan sát hậu điều kiện doanh thu — đã rà toàn bộ route `/api/admin/*`. `TC-API-LOGIN-024` có assertion Newman thật nhưng giữ nhãn `Blocked` vì tiền đề của nó (trạng thái sau khi counter reset) không tồn tại được trên SUT do D-LOGIN-06. Không case nào bị gán Pass/Fail suy diễn.
