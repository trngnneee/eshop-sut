# TC-ADMINUSERS-011: Reject delete with no Authorization header
...
## Test steps
1. DELETE /api/admin/users/{disposable_user_id} with no Authorization header.
2. GET /api/admin/users as admin: user still exists.
...
## Audit
- **Status:** INCOMPLETE
- **Reasoning:** Admin API requires a token (API spec §6; FR-12; SEC-02). HTTP 401 is not specified.
