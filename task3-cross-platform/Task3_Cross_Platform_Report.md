...
Each environment produced 58 result rows and 40 screenshots: 34 Pass, 23 Fail and 1 Not Observable. The combined dataset contains 232 rows and 160 PNG screenshots. Every row maps to an existing screenshot; every screenshot includes the student identity/email overlay, actual platform/engine, Windows/device declaration, SUT localhost URL, checklist IDs, observation and capture time.
...
|---|---|---|---|---:|---:|---:|---:|
| Google Chrome / Windows | YES | 150.0.7871.187 | Desktop 1440×900 | 34 | 23 | 1 | 40 PNG |
| Firefox / Windows | YES | 153.0 | Desktop 1440×900 | 34 | 23 | 1 | 40 PNG |
| Playwright WebKit / Windows | NO — not Safari | 26.5 | Desktop 1440×900 | 34 | 23 | 1 | 40 PNG |
| Chromium Pixel 7 emulation | NO — not real Android | 151.0.7922.34 | Emulated Pixel 7 | 34 | 23 | 1 | 40 PNG |

Across all runs, 58/58 item statuses were consistent. The eligible Chrome + Firefox subtotal is 116 rows: 68 Pass, 46 Fail and 2 Not Observable. The consistency result indicates that the failures are systemic rather than isolated to one tested browser; it must not be used to claim Safari/Android compatibility.

The 23 Fail verdicts are checklist comparisons, not automatically 23 software defects. In particular, inherited items `GUI-ADMIN-CATEGORY-005` and `GUI-ADMIN-CATEGORY-011` expect Category Edit and duplicate-name rejection, while the local FR-14 states Add/View/Delete and only requires a non-empty name. Their execution evidence is retained, but `Task3_Findings.md` classifies them as test-design/requirement mismatches rather than confirmed SUT bugs.
