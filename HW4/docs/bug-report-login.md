...
**106 passed / 31 failed / 137 total**. (No new bugs were found in the final boundary/robustness
volume pass - see `ai-review-login.md` §3e; every new case either confirmed correct handling of a
malformed input or reproduced an already-listed bug's failure pattern. A prior run of this suite
reported 32 failures: the shared seed account `test@eshop.com` was still locked from an earlier
back-to-back run within the SUT's real 180s lock window, so `TC-LOGIN-001` failed for that reason
alone, not a distinct defect - fixed with a `test.beforeAll` that force-unlocks the account before
this describe block runs; see `ai-review-login.md` §3g. Investigating it also surfaced a real
documentation gap: `BUG-FR02-A-13`, reproduced by `TC-JWT-001` all along, had never been added to
the table below - now item #13.)
...
> already-known bugs below (#1–#13) were never filed as GitHub Issues even in HW02 (confirmed:
...
| --- | ------------- | --------------------------------------------------------- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | BUG-FR02-A-01 | Failed-login counter increments by 2 instead of 1         | TC-LOGIN-002, TC-LOGIN-023, TC-LOGIN-024, TC-LOCK-BVA-009, `-010`, `-011`, `-017`, `-018`, `-021`, `-022` | `login_attempts` reads 2 after a single wrong password; every later-pass lockout boundary case (different `wrongAttempts`/`concurrentAttempts` values) fails against the same root cause - [screenshot](bug-evidence/BUG-FR02-A-01-counter-increments-by-2.png) |
...
| 13  | BUG-FR02-A-13 | JWT issued at login has no `exp` (expiration) claim        | TC-JWT-001                               | Decoded payload is `{iat, id, role}` — no `exp` key at all, so the token never expires - [screenshot](bug-evidence/BUG-FR02-A-13-jwt-missing-exp-claim.png)                                                                          |
