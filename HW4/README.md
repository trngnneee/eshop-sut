...
| **Total**       | **400** |     **1200** | **222** | **178** |                **31** |         **19** |

- **Features automated:** 3 (FR-02 Pool A, FR-07 Pool B, FR-13 Pool C).
- **Total bugs found by this automation:** 50 (31 already known from HW02, confirmed still
  ...
  **CRITICAL** privilege-escalation bug (`PUT /api/users/me` lets any user set their own `role`
  ...
  browser-login (see `docs/submission-checklist.md`). All 50 bugs total - the 31 already-known
  ...
  Asked why the 32 login failures didn't match its bug count, a case-by-case audit found a genuine
  flaky test (`TC-LOGIN-001`, fixed) and one documented-nowhere known bug (`BUG-FR02-A-13`, added)
  — see `ai-review-login.md` §3g for the full account.
...
Task 1 asks to _convert_ designed test cases into automation, not invent them from scratch: this
...
across all 3 engines, 50 real bugs found including 1 CRITICAL privilege-escalation vulnerability
