...
if ([string]::IsNullOrWhiteSpace($RepositoryRoot)) { $RepositoryRoot = Split-Path -Parent $taskRoot }
if ([string]::IsNullOrWhiteSpace($OutputPath)) { $OutputPath = Join-Path $taskRoot 'git-commit-log.txt' }

$RepositoryRoot = [System.IO.Path]::GetFullPath($RepositoryRoot)
$branch = (& git -C $RepositoryRoot branch --show-current)
$head = (& git -C $RepositoryRoot rev-parse HEAD)
$status = @(& git -C $RepositoryRoot status --short | Where-Object { $_ -notmatch 'task1-gui[\\/]git-commit-log\.txt$' })
$history = @(& git -C $RepositoryRoot log --date=iso-strict --pretty=format:'%H | %ad | %an | %s')
if ($LASTEXITCODE -ne 0) { throw "Unable to read Git history from $RepositoryRoot." }
...
$lines.Add('(The export target task1-gui/git-commit-log.txt is omitted from this section.)')
if ($status.Count -eq 0) { $lines.Add('(clean)') } else { foreach ($entry in $status) { $lines.Add($entry) } }
$lines.Add('')
$lines.Add('COMMIT HISTORY')
foreach ($entry in $history) { $lines.Add($entry) }
