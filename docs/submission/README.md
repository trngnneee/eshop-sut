# Báo Cáo Bài Tập HW03 - GUI & Usability

Tài liệu này là trang cấu trúc chính (README.md) cho bài tập cá nhân HW03 - Kiểm thử Giao diện (GUI) và Trải nghiệm người dùng (Usability) của dự án **EShop**.

---

## 1. Bảng Tự Đánh Giá (Self-Assessment Table)

Dưới đây là bảng tự đánh giá điểm số dựa trên các tiêu chí yêu cầu trong đề bài:

| STT | Tiêu chí đánh giá (Criteria) | Điểm tối đa | Điểm tự đánh giá (Self-Assessed Grade) | Minh chứng / Liên kết tài liệu |
|---|---|:---:|:---:|---|
| **1** | Task 1 — GUI Checklist (Thiết kế + Thực thi + Báo cáo lỗi) | 30 | `[Điểm số]` | [GUI_Checklist.md](file:///c:/Users/Public/Projects/Testing_HCMUS/HW3/eshop-sut/docs/submission/GUI_Checklist.md) <br> [Bug_Report.md](file:///c:/Users/Public/Projects/Testing_HCMUS/HW3/eshop-sut/docs/submission/Bug_Report.md) |
| **2** | Task 2 — Usability Evaluation (Kịch bản + 7 Phiên test + Phân tích) | 40 | `[Điểm số]` | [Usability_Session_Evidence.md](file:///c:/Users/Public/Projects/Testing_HCMUS/HW3/eshop-sut/docs/submission/Usability_Session_Evidence.md) |
| **3** | Task 3 — Cross-Browser / Cross-Platform (Tối thiểu 3 nền tảng) | 20 | `[Điểm số]` | [Cross_Platform_Evidence.md](file:///c:/Users/Public/Projects/Testing_HCMUS/HW3/eshop-sut/docs/submission/Cross_Platform_Evidence.md) |
| **4** | Agent Skills (Kỹ năng Agent đã xây dựng + Video minh họa) | 10 | `[Điểm số]` | [Main_Report.md#agent-skills](file:///c:/Users/Public/Projects/Testing_HCMUS/HW3/eshop-sut/docs/submission/Main_Report.md#4-agent-skills) |
| | **Tổng cộng (Total)** | **100** | **`[Tổng điểm]`** | |

---

## 2. Báo Cáo Tóm Tắt Kiểm Thử (Test Summary Report)

- **Số lượng màn hình / Luồng kiểm thử đã test:** `[Số lượng]` (Ví dụ: 2 màn hình Home & Cart / Luồng Đăng ký -> Mua hàng)
- **Tổng số checklist item đã thiết kế:** `[Số lượng]` (Yêu cầu > 40 items)
- **Kết quả thực thi checklist:**
  - **Đạt (Passed):** `[Số lượng]`
  - **Không đạt (Failed):** `[Số lượng]`
- **Tổng số lỗi (Bugs) phát hiện:** `[Số lượng]`
- **Số lượng người tham gia đánh giá Usability (Participants):** `7` người
- **Đường dẫn video demo sử dụng Agent Skill (Youtube Link):** `[Link video]`

---

## 3. Bản Đồ Thư Mục Nộp Bài (Deliverables Map)

Vui lòng nhấp vào các liên kết bên dưới để xem chi tiết từng thành phần bàn giao:

1. **Báo cáo chính (Main Report):** [Main_Report.md](file:///c:/Users/Public/Projects/Testing_HCMUS/HW3/eshop-sut/docs/submission/Main_Report.md)
2. **Báo cáo lỗi chi tiết (Bug Report):** [Bug_Report.md](file:///c:/Users/Public/Projects/Testing_HCMUS/HW3/eshop-sut/docs/submission/Bug_Report.md)
3. **Phê bình & Nhật ký Kiểm toán AI (AI Critique & Audit Report):** [AI_Report.md](file:///c:/Users/Public/Projects/Testing_HCMUS/HW3/eshop-sut/docs/submission/AI_Report.md)
4. **Lịch sử Git commit (Git Commit Log):** [Git_Commit_Log.txt](file:///c:/Users/Public/Projects/Testing_HCMUS/HW3/eshop-sut/docs/submission/Git_Commit_Log.txt)
5. **Checklist kiểm thử GUI (GUI Checklist):** [GUI_Checklist.md](file:///c:/Users/Public/Projects/Testing_HCMUS/HW3/eshop-sut/docs/submission/GUI_Checklist.md)
6. **Bằng chứng Usability Sessions (Usability Session Evidence):** [Usability_Session_Evidence.md](file:///c:/Users/Public/Projects/Testing_HCMUS/HW3/eshop-sut/docs/submission/Usability_Session_Evidence.md)
7. **Ảnh chụp màn hình Đa nền tảng (Cross-Platform Evidence):** [Cross_Platform_Evidence.md](file:///c:/Users/Public/Projects/Testing_HCMUS/HW3/eshop-sut/docs/submission/Cross_Platform_Evidence.md)
