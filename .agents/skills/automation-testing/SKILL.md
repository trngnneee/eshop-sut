...
## PHASE 3 — Review & Fix (Human Review)

Sau khi có script AI-generated, thực hiện kiểm tra theo checklist sau:

### 3.1 Checklist Review

| Hạng mục | Kiểm tra | Trạng thái |
|:---------|:---------|:----------:|
| Selector stability | Không dùng XPath phức tạp, text fragile | [ ] |
| Wait strategy | Không dùng `page.waitForTimeout()` | [ ] |
| Assertion coverage | Ít nhất 3 pattern khác nhau | [ ] |
| Data-driven | Data đọc từ file JSON/CSV, không hardcode | [ ] |
| Edge cases | Có negative + edge cases | [ ] |
| Run by annotation | Mỗi test có `Run by: 23127486` | [ ] |
| Error messages | Assert messages có ý nghĩa | [ ] |

### 3.2 Ghi lại issues vào MainReport.md

Với mỗi issue phát hiện, ghi vào bảng "Human Review & Fixes" trong `MainReport.md`:

```markdown
| Issue | Description | Fix Applied |
|:------|:------------|:------------|
| Fragile selector | AI dùng `text="Đăng ký"` dễ vỡ khi dịch | Đổi sang `[data-testid="register-btn"]` |
| Missing wait | Không có wait sau khi submit form | Thêm `await expect(successMsg).toBeVisible()` |
| Weak assertion | Chỉ check visibility, không check nội dung | Thêm `toContainText('Đăng ký thành công')` |
```

---

## PHASE 4 — Chạy Test trên 3 Browsers
...
### 4.3 Kiểm tra report có "Run by: 23127486"
...
## PHASE 6 — Cập nhật tài liệu nộp bài (QUAN TRỌNG)

Sau khi hoàn thành test, **bắt buộc** cập nhật các file trong `submission/`:

### 6.1 Cập nhật `submission/MainReport.md`
...
### 6.2 Cập nhật `submission/AI_Audit.md`
...
### 6.3 Cập nhật `submission/AI_Critique.md`
...
### 6.4 Cập nhật `submission/Bug_Report.md`
...
### 6.5 Cập nhật `submission/README.md`
...
### 6.6 Cập nhật `submission/git_commit_log.txt`

Sau khi commit các test script lên GitHub:

```powershell
git log --pretty=format:"%h | %ad | %an | %s" --date=short -- "tests/**/*.spec.ts" "tests/**/*.spec.js"
```

Paste output vào file `git_commit_log.txt`, thay thế phần placeholder.

---

## PHASE 7 — Kiểm tra hoàn thành
