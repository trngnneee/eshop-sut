...
def analyze_jtl(jtl_path, out_dir=None, scenario_name=None, slice_sec=60, resource_csv=None):
    ...
    slice_data = {f"{s*slice_sec}-{(s+1)*slice_sec}s": {'elapsed': [], 'errors': 0, 'threads': []} for s in range(num_slices)}

    for t, e, ath in zip(all_timestamps, all_elapsed, all_threads_list):
