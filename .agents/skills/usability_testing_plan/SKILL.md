...
Trước khi tiến hành, Agent/Người dùng cần chuẩn bị sẵn hai công cụ chính:
...
- **UEQ-S (User Experience Questionnaire - Short):** Gồm 8 cặp tính từ đối lập (ví dụ: cản trở vs. hỗ trợ, phức tạp vs. dễ dùng).
...
Khi quan sát người dùng thực hiện tác vụ, hãy ghi chép lại theo cấu trúc sau (không tự bịa kết quả khi thiết kế kế hoạch):
...
- **Mục 2 (Bảng danh sách 7 người tham gia):** Điền danh sách 7 người dùng đã tuyển chọn (họ tên, đối tượng, SĐT đã che 4 số giữa, phương thức liên lạc, trạng thái buổi test).
- **Mục 3 (Ghi chép quan sát):** Dựng sẵn sườn sườn trống cho 7 buổi test theo cấu trúc ở phần 5 để sẵn sàng ghi nhận dữ liệu trong quá trình chạy kiểm thử thực tế.
