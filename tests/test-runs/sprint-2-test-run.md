...
| [TC-LOGIN-021](../test-cases/login/TC-LOGIN-021.md) | Login | AI Tester | Fail | #45 | Thứ tự tab di chuyển bị lỗi (nút submit focus trước) và thiếu hoàn toàn aria-label trên các input. |
| [TC-LOGIN-022](../test-cases/login/TC-LOGIN-022.md) | Login | AI Tester | Fail | #45 | Các input thiếu thuộc tính autocomplete và name tiêu chuẩn tương thích trình quản lý mật khẩu. |
...
| [TC-LOGIN-029](../test-cases/login/TC-LOGIN-029.md) | Login | AI Tester | Fail | #47 | Email bị so khớp phân biệt hoa thường ở SQLite (toán tử =), gây lỗi 401 khi khác casing và không đếm sai/khóa. |
...
| [TC-LOGIN-031](../test-cases/login/TC-LOGIN-031.md) | Login | AI Tester | Fail | #48 | API reset mật khẩu thành công nhưng không giải phóng `locked_until` và `login_attempts` trong CSDL. |
...
17. **Bug #48 (Email phân biệt hoa thường):** Ở `backend/server.js:35`, truy vấn DB so khớp email bằng `=` (phân biệt hoa thường trong SQLite mặc định) khiến email viết hoa/thường xen kẽ đăng nhập thất bại và cơ chế khóa không hoạt động.
18. **Bug #49 (Reset mật khẩu không mở khóa):** Ở `backend/server.js:89`, API cập nhật mật khẩu mới chỉ set `password` và `reset_token = NULL`, không reset `login_attempts` and `locked_until`. Tài khoản vẫn bị khóa sau khi đổi mật khẩu thành công.
