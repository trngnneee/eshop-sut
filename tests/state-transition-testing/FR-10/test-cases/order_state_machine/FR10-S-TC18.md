# FR10-S-TC18: Từ chối status không nằm trong state machine: refund
...
| Thuộc tính | Giá trị |
| :--- | :--- |
| Transition / Class ID | S-INVALID-11 |
| State variable | `status` |
| Actor | admin |
| Flow type | Invalid transition |
| Covered guard/rule | Invalid status value: refund |
...
- Execution interface: Local API `PUT /api/admin/orders/18/status` with SQLite fixture order `18`.
