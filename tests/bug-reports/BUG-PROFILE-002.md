Title: [BUG][Profile] Thiếu validate kiểm tra rỗng cho trường Họ Tên (name)
...
2. Gửi request PUT đến endpoint `/api/users/me` với trường `name: ""`:
   ```http
...
## Expected result
Hệ thống Hệ thống từ chối cập nhật, trả về HTTP 400 Bad Request và báo lỗi Họ Tên không được phép để trống.

## Actual result
Hệ thống chấp nhận cập nhật, trả về HTTP 200 OK và cập nhật Họ Tên thành chuỗi rỗng `""` trong database.

## Evidence
![Ảnh lỗi API/Database](../bugs-screenshots/BUG-PROFILE-002.png)
