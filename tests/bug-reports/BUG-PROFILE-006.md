Title: [BUG][Profile] Thiếu validate kiểm tra ký tự đặc biệt trong trường Họ Tên (name)
...
2. Gửi request PUT đến endpoint `/api/users/me` với trường `name: "Nguyen@Van_A"`:
   ```http
...
## Expected result
Hệ thống Hệ thống từ chối cập nhật, trả về HTTP 400 Bad Request và báo lỗi Họ Tên không hợp lệ (không được chứa ký tự đặc biệt).

## Actual result
Hệ thống chấp nhận cập nhật, trả về HTTP 200 OK và lưu Họ Tên chứa ký tự đặc biệt vào database.

## Evidence
![Ảnh lỗi API/Database](../bugs-screenshots/BUG-PROFILE-006.png)
