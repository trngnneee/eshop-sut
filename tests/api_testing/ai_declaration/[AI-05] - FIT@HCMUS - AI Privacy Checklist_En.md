...
**HW06 note — SUT is a deliberately vulnerable demo.** The System Under Test (EShop `eshop-sut`) is a course-provided demo app that intentionally contains bugs; it holds only seeded test data (`admin@eshop.com`, `test@eshop.com`) — no real personal data. My security tests (SQL Injection, UNION credential leak) were run only against my own local instance (localhost:3000), never against any third-party or production system. The `X-Student-Id` header carries my own student ID only. The AI-test-generator diagram required by the "Create" level (G9.5) is self-drawn by me — NOT AI-generated — per the assignment's anti-AI-cheat constraint.
