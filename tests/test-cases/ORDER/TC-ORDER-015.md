...
Kiểm tra hệ thống từ chối chuyển đổi ngược không hợp lệ từ `shipping` quay lại `confirmed`.
...
4. Cố gắng chuyển trạng thái quay lại `confirmed`
...
Hệ thống từ chối yêu cầu, trả về HTTP 400 và thông báo lỗi: `Invalid state transition from shipping to confirmed`. Trạng thái đơn hàng giữ nguyên là `shipping`.
