...
- Name: fr19-user-management.spec.js >> FR-19 - Quản lý người dùng admin >> TC-FR19-11 - User thường không được xóa user
- Location: tests\fr19-user-management.spec.js:346:3
...
> 366 |     expect(deleteResponse.status()).toBe(expectedData.expectedStatus);
  ...
  442 |     }
  443 | 
  444 |     for (const forbiddenCellValue of expectedData.forbiddenCellValues) {
  445 |       await expect(adminPage.usersTable).not.toContainText(forbiddenCellValue);
  446 |     }
  447 |   });
  448 | });
  449 | 
