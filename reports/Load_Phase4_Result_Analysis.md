...
| Add per-thread or per-user test accounts instead of reusing only `test@eshop.com` for all load threads. | Plausible but not proven | `auth_users.csv` repeats one account; workflow uses cart and orders tied to `user_id`; sampler counts are uneven from 292 login to 283 My Orders | Reduce shared-user state interference, make order-history assertions cleaner, and improve repeatability for higher Stress/Spike loads. |
